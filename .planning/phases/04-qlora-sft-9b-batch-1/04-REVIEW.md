...
**Files Reviewed:** 12  
**Status:** fixed
**Fix verification:** `./.venv/bin/python -m pytest tests/test_v3_sft_config.py tests/test_v3_sft_arrow_loader.py tests/test_v3_sft_dry_run.py tests/test_v3_sft_grad_gate.py tests/test_v3_sft_frozen.py tests/test_v3_sft_artifacts.py -q` → 38 passed; `bash -n scripts/run_v3_phase4_dry_run.sh scripts/run_v3_phase4_full.sh` → passed.

## Summary

审查重点覆盖 Phase 4 SFT 配置、训练入口、dry-run/full-run gate、聚合报告和 DGX Spark wrapper。原始审查发现的 7 个 Critical 和 1 个 Warning 已通过 fail-closed 实现与回归测试修复。Phase 4 仍不能直接进入下游 Phase 5，因为用户已选择不启动长时 full-run，真实 green full-run manifest 尚未产生。
...
**Status:** Fixed in `tsc_cycle/student/sft_v3.py` with LoRA-only trainable parameter evidence.
...
**Status:** Fixed in `tsc_cycle/student/train.py`, `tsc_cycle/v3_gates/sft_report_v3.py`, and artifact regression tests.
...
**Status:** Fixed in `tsc_cycle/v3_gates/sft_dry_run_v3.py` with raw SOLUTION JSON parsing and non-integer regression coverage.
...
**Status:** Fixed in `scripts/run_v3_phase4_dry_run.sh` and `tsc_cycle/v3_gates/sft_dry_run_v3.py`; elapsed budget now includes OOD generation/lint.
...
**Status:** Fixed in `tsc_cycle/v3_gates/sft_report_v3.py` with missing pass-rate regression coverage.
...
**Status:** Fixed in `tsc_cycle/v3_gates/sft_report_v3.py` with finite loss/grad evidence regression coverage.
...
**Status:** Fixed in `tsc_cycle/v3_gates/sft_report_v3.py` with adapter/checkpoint file regression coverage.
...
**Status:** Fixed in both Phase 4 wrappers and `tsc_cycle/student/train.py` with project-root anchored v1 path.
