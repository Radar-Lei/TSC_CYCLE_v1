...
## Current Milestone: v1.1 Improve Reward & GRPO Data Filter

**Goal:** 解决 GRPO 训练中 reward 二值化（0 或满分）和无效样本问题，让 reward 信号更有区分度，数据集更干净。

**Target features:**
- 改进 SUMO reward 公式，消除二值化问题（去掉 cap + 非线性压缩）
- 改进 baseline 计算策略（从默认周期改为饱和度启发式基准）
- 过滤空交叉口样本（baseline passed=0 且 queue=0）
- 过滤低区分度样本
- 场景均衡抽样
- 解决 zero-std 无效训练步问题
...
- [ ] 改进 SUMO reward 公式和 baseline 基准
- [ ] GRPO 数据过滤（空交叉口、低区分度、场景均衡）
- [ ] 解决 zero-std 无效训练步问题
