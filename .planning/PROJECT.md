...
## Current Milestone: v1.2 简化版 GRPO 训练与验证

**Goal:** 在本机跑通简化版 GRPO 真实训练，并用自动验证脚本确认产出模型满足格式约束和饱和度比例分配。

**Target features:**
- 用现有 Docker 链路完成一次简化版 GRPO 真实训练，产出模型和训练日志
- 编写自动验证脚本：喂测试数据、解析模型输出、检查格式/约束/饱和度比例
- 验证脚本输出机器可读的通过/失败结论

## Current State

**Shipped:** v1.0 (2026-04-01), v1.1 (2026-04-02)
...
当前缺口：
- 简化版 GRPO 还没有完成一次真实训练产出并沉淀可复用模型
- 训练完成后缺少自动验证脚本来检查格式约束和饱和度比例分配
...
- [ ] 开发者可以用现有 Docker 链路完成一次简化版 GRPO 真实训练，产出模型和训练日志
- [ ] 开发者可以运行自动验证脚本，喂测试数据给训练产出的模型并检查格式/约束/饱和度比例
- [ ] 验证脚本输出机器可读的通过/失败结论
...
---
*Last updated: 2026-04-02 after redefining v1.2 milestone*
