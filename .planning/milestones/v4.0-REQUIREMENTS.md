# Requirements Archive: v4.0 4B 回退 + 扩展数据重训 + 标签协议修复
...
- [x] **BASE-01**: 学生模型固定为 `Qwen/Qwen3-4B-Thinking-2507`，不再使用 Qwen3.5-9B 作为本里程碑训练基座
- [x] **BASE-02**: 训练栈沿用 `/dgx-spark-training` 与 `/home/samuel/dgx-spark-setup/.venv` 的已验证 4B 路径，不升级 PyTorch/Transformers/训练框架
- [x] **BASE-03**: v1.0 baseline artifact 与 gen_cache 以只读方式引用，v4.0 流程不得写入 `runs/20260507T032419Z/`

### TAG — 标签协议修复

- [x] **TAG-01**: 全链路思考协议固定为 `<start_working_out>...</end_working_out><SOLUTION>...</SOLUTION>`
- [x] **TAG-02**: 数据清洗/重建必须把错误的 `<end_working_out>` 结束标签规范化为 `</end_working_out>`，并报告替换数量
- [x] **TAG-03**: 训练与评测 prompt builder 禁止注入原生 `<think>`/`</think>` 或 chat_template thinking token
- [x] **TAG-04**: Qwen3-4B tokenizer audit 证明 4 个自定义标签均为多 sub-token，且 native `<think>`/`</think>` token id 被动态记录用于泄漏检查
...
- [x] **GGUF4B-02**: HF / GGUF fp16 / GGUF q4_K_M 三精度 smoke 均输出完整 `<start_working_out>...</end_working_out><SOLUTION>...</SOLUTION>` 结构
- [x] **GGUF4B-03**: HF tokenizer ↔ llama-tokenize parity 在固定 prompt fixture 上通过
- [x] **GGUF4B-04**: q4_K_M 相对 HF/fp16 的格式与硬约束 smoke 不崩塌；失败时记录 q5_K_M fallback 决策点
...
- [x] **EVAL4B-01**: 评测矩阵至少包含 v4 HF、v4 q4_K_M、v1 q4_K_M baseline，并保持 v1 baseline read-only 不重生成
- [x] **EVAL4B-02**: 报告 OOD hard-constraint pass、teacher MAE、format pass、q4-vs-HF ratio、bootstrap CI、p99/max-abs tail metrics
- [x] **EVAL4B-03**: 决策门要求 v4 q4_K_M hard-constraint pass ≥98%，q4-vs-HF ratio ≥0.95，且相对 v1 baseline 不显著回退
- [x] **EVAL4B-04**: `decision.md` 明确给出 GO / NO-GO / 用户决策，并解释扩展数据与 `</end_working_out>` 标签修复的贡献
...
| GGUF4B-02 | Phase 10 | Complete |
| GGUF4B-03 | Phase 10 | Complete |
| GGUF4B-04 | Phase 10 | Complete |
| EVAL4B-01 | Phase 11 | Complete |
| EVAL4B-02 | Phase 11 | Complete |
| EVAL4B-03 | Phase 11 | Complete |
| EVAL4B-04 | Phase 11 | Complete |
