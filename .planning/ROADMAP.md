...
- [x] **Phase 9: 4B QLoRA retrain** — 沿用 v1 已验证 4B QLoRA 路线，在 v4 数据与正确标签协议上完成重训
- [x] **Phase 10: merge + GGUF export** — 将 v4 adapter merge 并导出 GGUF fp16 与 q4_K_M，验证三精度协议与 tokenizer parity
- [x] **Phase 11: eval matrix + decision** — 对比 v4 HF/q4 与 v1 q4 baseline，给出 GO/NO-GO/用户决策
...
- [x] 09-02-PLAN.md — 实现 4B/v4 SFT helper、pre-train smoke evaluator 与 Phase 9-aware trainer mode。
- [x] 09-03-PLAN.md — 实现 Phase 9 smoke/full-training wrappers 与 fail-closed aggregate report evaluator。
- [x] 09-04-PLAN.md — 自动执行 smoke gate、run_safe full training 与最终 Phase 9 aggregate handoff report。
...
- [x] 10-03-PLAN.md — 实现并运行固定 prompt fixture 的 HF tokenizer ↔ llama-tokenize parity 与 custom-tag GGUF metadata sanity gate。
- [x] 10-04-PLAN.md — 实现并运行 HF/GGUF fp16/GGUF q4_K_M 三精度协议与硬约束 smoke，生成最终 Phase 10 aggregate report。
...
**Plans**:
- [x] 11-01-PLAN.md — 建立 Phase 11 RED 合约，锁定 v1 只读、backend normalization、bootstrap/tail stats、决策阈值与 decision markdown。
- [x] 11-02-PLAN.md — 实现 Phase 11 matrix builder 与 fixed wrapper，生成 v4 HF/q4 eval caches 并只读引用 v1 baseline。
- [x] 11-03-PLAN.md — 实现 metrics.json/report.md，覆盖 OOD hard pass、teacher MAE、format pass、q4-vs-HF ratio、bootstrap CI 与 tail metrics。
- [x] 11-04-PLAN.md — 实现 GO/NO-GO/USER_DECISION gate、decision.md 与 phase11_gate_report.json handoff。
...
### Phase 12: 需要用最新训练好的模型，以 /home/samuel/TSC_CYCLE/reality.log 的输入为输入（忽略其输出，以我们自己的模型输出为输出，要包括思考过程），构成一个reality_test.log

**Goal:** Use the latest trained model, take `/home/samuel/TSC_CYCLE/reality.log` inputs as inputs, ignore its existing outputs, and create `/home/samuel/TSC_CYCLE/reality_test.log` whose outputs are generated by our model and include reasoning. Preserve `<start_working_out>...</end_working_out><SOLUTION>...</SOLUTION>` and forbid native `<think>` tags.
**Requirements**: PHASE12-GOAL
**Depends on:** Phase 11
**Plans:** 3/3 plans executed

Plans:
**Wave 1**
- [x] 12-01-PLAN.md — 建立 Phase 12 RED 契约，锁定 reality.log input-only parser、协议、路径安全、报告与原子写入行为。

**Wave 2** *(blocked on Wave 1 completion)*
- [x] 12-02-PLAN.md — 实现可复用 Phase 12 CLI/wrapper，解析 reality.log、调用 Phase 11 GO GGUF 模型、缓存输出、验证并原子写入。

**Wave 3** *(blocked on Wave 2 completion)*
- [x] 12-03-PLAN.md — 执行完整 426 条 reality replay，生成 reality_test.log 与 phase12_report.json，并进行最终门禁/人工抽查。
