...
def check_phase8_handoff(phase8_report: str | os.PathLike[str] = PHASE8_GATE_REPORT) -> dict[str, Any]:
    path = Path(phase8_report)
    if not path.exists():
        return {"ok": False, "next_phase_allowed": False, "fatal_failures": [{"gate": "phase8_handoff", "reason": f"missing {path}"}]}
    payload = json.loads(path.read_text(encoding="utf-8"))
    required = {"DATA4B-01", "DATA4B-02", "DATA4B-03", "DATA4B-04", "DATA4B-05"}
    ...
    if not ok:
        ...
        payload.setdefault("fatal_failures", []).append({"gate": "phase8_handoff", "reason": "Phase 8 handoff is not green or lacks DATA4B coverage"})
        payload["ok"] = False
    return payload
...
def build_sample_format_audit(tokenized_dir: Path = TOKENIZED_DIR, phase8_report: Path = PHASE8_GATE_REPORT) -> dict[str, Any]:
    phase8 = check_phase8_handoff(phase8_report)
    splits: dict[str, int] = {}
    ...
    for split in ("train", "val", "ood_val"):
        ...
        if "v3" in path.as_posix() or "20260507T032419Z" in path.as_posix():
            forbidden_paths.append(path.as_posix())

    if phase8.get("ok") is not True or phase8.get("next_phase_allowed") is not True:
        fatal_failures.append({"gate": "phase8_handoff", "reason": "Phase 8 gate is not green"})
    if forbidden_paths:
    ...
    return {
        ...
        "phase8_gate": {"ok": phase8.get("ok"), "next_phase_allowed": phase8.get("next_phase_allowed")},
...
def data_manifest_sha256(phase8_report: str | os.PathLike[str] = PHASE8_GATE_REPORT) -> str:
    return _sha256_file(Path(phase8_report))
