...
def test_malformed_progress_jsonl_fails_before_api_call(tmp_path: Path):
    sample = _sample("2026-05-02 00:00:00")
    input_path = _write_jsonl(tmp_path / "inputs.jsonl", [sample])
    old_labeled = tmp_path / "old_labeled.jsonl"
    old_labeled.write_text('{"sample_id": "done"\n', encoding="utf-8")
    labeled = tmp_path / "labeled_new.jsonl"
    rejected = tmp_path / "rejected_new.jsonl"
    fake = FakeClient()
    ...
    with pytest.raises(ValueError, match=rf"malformed progress JSONL at {old_labeled}:1"):
        run_labeling(args, client_factory=lambda **_: fake)
...
def test_missing_sample_id_progress_row_fails_before_api_call(tmp_path: Path):
    sample = _sample("2026-05-02 00:00:00")
    input_path = _write_jsonl(tmp_path / "inputs.jsonl", [sample])
    old_labeled = _write_jsonl(tmp_path / "old_labeled.jsonl", [{"result": {"success": True}}])
    labeled = tmp_path / "labeled_new.jsonl"
    rejected = tmp_path / "rejected_new.jsonl"
    fake = FakeClient()
    ...
    with pytest.raises(ValueError, match=rf"progress row missing sample_id at {old_labeled}:1"):
        run_labeling(args, client_factory=lambda **_: fake)
