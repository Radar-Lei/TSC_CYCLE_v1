...
def test_phase12_contracts_do_not_import_heavy_model_stacks_at_collection() -> None:
    ...
    assert imported_roots.isdisjoint(FORBIDDEN_COLLECTION_IMPORTS), (
        "Phase 12 contracts must use lazy imports and must not import torch/transformers/peft/"
        "bitsandbytes/vllm/flash_attn during collection"
    )
    forbidden_student_module = "tsc_cycle." + "student"
    assert forbidden_student_module not in source, "Phase 12 RED contracts must target lightweight v4_gates modules"
...
def test_report_evaluation_fails_closed_on_parse_lint_count_or_hash_failures(tmp_path: Path) -> None:
    ...
    base_kwargs = {
        ...
        "model_artifact": APPROVED_Q4_MODEL,
        "model_sha256": "model-hash",
        "input_sha256": "input-log-hash",
        "output_sha256": "output-log-hash",
        "out_path": tmp_path / "phase12_report.json",
    }
    ...
    assert passing["model_artifact"] == str(APPROVED_Q4_MODEL)
    for key in ("input_sha256", "output_sha256"):
        assert passing[key]

    failing_cases = {
        "parse_error": {**base_kwargs, "outputs": [{**_good_output(), "parse_error": "solution_unparseable", "solution": None}]},
        "lint_false": {**base_kwargs, "outputs": [{**_good_output(), "lint_ok": False, "lint": {"ok": False, "violations": ["below_min"]}}]},
...
def test_report_payload_contains_phase12_audit_fields(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "phase12_report.json").exists()
...
def test_final_log_is_not_written_when_any_gate_fails(tmp_path: Path) -> None:
    ...
    failed_outputs = [{**_good_output(), "parse_error": "solution_unparseable", "solution": None}]
